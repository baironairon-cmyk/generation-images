...
async function downloadTelegramFile(fileId: string): Promise<{ path: string; mime?: string; width?: number; height?: number }> {
  ...
  const filePath = getPathResp.data?.result?.file_path as string;
...
export function createBot() {
  ...
  bot.on('photo', async (ctx) => {
    try {
      ...
      const results = [] as { path: string; label: string }[];
      ...
      const media = results.map((r, idx) => ({ type: 'photo' as const, media: Input.fromLocalFile(r.path), caption: `#${idx+1} ${r.label}` }));
